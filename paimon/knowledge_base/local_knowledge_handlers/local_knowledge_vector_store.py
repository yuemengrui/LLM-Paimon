...
from knowledge_base.loader.file_loader import load_file
from knowledge_base.vector_stores.my_faiss import MyFAISS
from api_servers import get_embeddings
...
class KnowledgeVectorStore:
    ...
    def get_doc_info(self, vector_dir):
        sentences = []
        vector_store = MyFAISS.load_local(vector_dir)
        for i in range(len(vector_store.index_to_docstore_id)):
            sentences.append(vector_store.docstore.search(vector_store.index_to_docstore_id[i]).page_content)

        return sentences

    def build_vector_store(self, file_path: str, file_hash: str, vector_dir: str, embedding_model_name: str):
        ...
        if not os.path.exists(file_path):
            ...
            embeddings = get_embeddings(sentences=sentences, model_name=embedding_model_name)
    ...
    def generate_knowledge_based_prompt(self, embedding_model_name, query, vector_store_dir_list,
        ...
        embedding = get_embeddings(sentences=[query], model_name=embedding_model_name)[0]
